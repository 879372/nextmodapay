...
export default function ExampleComponent() {
    ...
    const handleNextPage = () => {
        setPaginaAtual(prev => Math.min(prev + 1, pagination - 1)); 
    };
    ...
    const exportToExcel = async () => {
        ...
        const ws = XLSX.utils.json_to_sheet(allTransactions.map((transacao) => ({
            ...
            'Valor Pago': `R$ ${parseFloat(String(transacao.pagador.valor)).toFixed(2)}`,
            Status: transacao.status,
            Pagador: transacao.pagador.nome,
            'Data Pagamento': new Date(transacao.pagador.data).toLocaleDateString(),
            ...
            'Data Devolução': transacao.devolucao.data ? new Date(transacao.devolucao.data).toLocaleDateString() : ''
    ...
    return (
      ...
      <div className={`flex-1 transition-margin duration-300 ease-in-out ${isSmallScreen ? 'ml-0' : (isOpen ? 'ml-64' : 'ml-0')}`} style={{ width: isOpen ? 'calc(100% - 256px)' : '100%' }}>
            <Header titulo="Pix In" isOpen={isOpen} toggleSidebar={toggleSidebar} />
